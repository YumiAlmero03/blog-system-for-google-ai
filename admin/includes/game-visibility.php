...
{
    return "$alias.published = 1 AND $alias.done_processing = 1 AND (" . game_public_slug_sql($alias) . ') AND (' . games_ph_allowed_sql($alias . '.restrictions') . ') AND (' . game_visibility_sql($alias) . ')';
}
...
function game_provider_approval_save(PDO $pdo, string $key, mixed $approved): void
{
    ...
    if (!$tracked) { require_once __DIR__ . '/indexing-queue.php'; indexing_content_changed($pdo, 'game'); $tracked=true; }
