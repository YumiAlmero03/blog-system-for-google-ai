...
try {
    ...
    $pdo->exec("CREATE TABLE index_checks (id INTEGER PRIMARY KEY, url TEXT UNIQUE, sitemap_url TEXT, content_type TEXT, in_sitemap INTEGER DEFAULT 1, queued INTEGER DEFAULT 0, created_at INTEGER, updated_at INTEGER)");
    $pdo->prepare("INSERT INTO index_checks(url,content_type,queued) VALUES(?,'page',1)")->execute([public_url('/contact/')]);
    generate_game_sitemaps($db, $dir);
    check((int)$pdo->query("SELECT COUNT(*) FROM index_checks WHERE content_type='game' AND in_sitemap=1")->fetchColumn() === 2005, 'Checker membership populated');
    $pdo->exec("UPDATE games SET provider_slug='bulk' WHERE slug LIKE 'filler-%'");
    $pdo->exec("INSERT INTO game_provider_settings VALUES('slug:bulk',0,0)");
    $pdo->exec("UPDATE index_checks SET queued=1 WHERE content_type='game'");
    $reduced = generate_game_sitemaps($db, $dir);
    check($reduced['eligible'] === 5 && $reduced['chunks'] === 1, 'Provider exclusion controls chunk count');
    check(!is_file($dir.'/sitemap-games-2.xml'), 'Approval shrink removes chunks');
    check((int)$pdo->query("SELECT COUNT(*) FROM index_checks WHERE content_type='game' AND in_sitemap=0 AND queued=0")->fetchColumn() === 2000, 'Hidden URLs inactive and unqueued');
    check((int)$pdo->query("SELECT queued FROM index_checks WHERE content_type='page'")->fetchColumn() === 1, 'Page membership and queue preserved');
    $pdo->exec("UPDATE game_provider_settings SET approved=1");
    check(generate_game_sitemaps($db, $dir)['eligible'] === 2005, 'Reapproval restores games');
    check((int)$pdo->query("SELECT COUNT(*) FROM index_checks WHERE content_type='game' AND in_sitemap=1")->fetchColumn() === 2005, 'Checker URLs reactivated');
    $pdo->exec("UPDATE games SET is_viewable=0 WHERE slug='filler-0'");
    $pdo->exec("UPDATE games SET restrictions='PH' WHERE slug='filler-1'");
    check(generate_game_sitemaps($db, $dir)['eligible'] === 2003, 'Hidden and changed PH exclusion');
    check((int)$pdo->query('SELECT COUNT(*) FROM games')->fetchColumn() === 2016, 'Visibility preserves game records');
