...
function admin_slots_page_url(int $page): string
{
    ...
    return '/admin/games/?' . http_build_query($params);
}
...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
    ...
    if (isset($_POST['return_to']) && is_string($_POST['return_to']) && str_starts_with($_POST['return_to'], '/admin/games/')) {
    ...
    <main id="main-content" style="padding: 20px 16px;">
      <div class="admin-container">
        <div class="admin-header">
          <div>
            <h1 style="font-size:1.6rem; color:var(--brand-dark);">Games</h1>
            <p style="font-size:0.88rem; color:var(--text-muted);">Browse imported SlotsLaunch games, providers, and publish status.</p>
          </div>
          <div style="display:flex; gap:8px; flex-wrap:wrap;">
            <a href="/admin/games/" class="btn btn-secondary btn-sm">Reset</a>
          </div>
        </div>

        <div class="slot-metrics">
          <div class="slot-metric"><strong><?= number_format($total) ?></strong><span>Filtered Games</span></div>
          <div class="slot-metric"><strong><?= number_format((int) $pdo->query('SELECT COUNT(*) FROM games')->fetchColumn()) ?></strong><span>Total Games</span></div>
        ...
        <form class="slot-filters" method="get" action="/admin/games/">
        ...
        <?php if ($slots === []): ?>
          <div class="slot-empty">No games found. Import games with <code>php admin/games/scripts/import-slotslaunch-games.php</code>.</div>
        <?php else: ?>
          <?php foreach ($slots as $slot): ?>
            ...
            <article class="slot-card">
              ...
              <div class="slot-actions">
                <form class="slot-featured-form" method="post" action="/admin/games/">
                  <?= csrf_input() ?>
                  <input type="hidden" name="action" value="visibility">
                  <input type="hidden" name="slot_id" value="<?= (int)$slot['id'] ?>">
                  <input type="hidden" name="return_to" value="<?= h($_SERVER['REQUEST_URI'] ?? '/admin/games/') ?>">
                ...
                <a class="btn btn-secondary btn-sm" href="/admin/games/edit.php?id=<?= h($slot['id']) ?>">Edit Content</a>
        ...
        <?php if ($totalPages > 1): ?>
          <nav class="slot-pagination" aria-label="Games pagination">
