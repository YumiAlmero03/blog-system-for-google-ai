...
function game_provider_approved_sql(string $alias = 'games'): string
{
    $key = game_provider_key_sql($alias);
    return "NOT EXISTS (SELECT 1 FROM game_provider_settings gps WHERE gps.provider_key = ($key) AND gps.approved = 0)";
}

function game_visibility_sql(string $alias = 'games'): string
{
    return "$alias.is_viewable = 1 AND (" . game_provider_approved_sql($alias) . ')';
}

function game_public_eligibility_sql(string $alias = 'games'): string
{
    return "NOT EXISTS (SELECT 1 FROM game_module_settings WHERE id=1 AND enabled=0) AND $alias.published = 1 AND $alias.done_processing = 1 AND (" . game_public_slug_sql($alias) . ') AND (' . games_ph_allowed_sql($alias . '.restrictions') . ') AND (' . game_visibility_sql($alias) . ')';
}

function game_public_slug_sql(string $alias = 'games'): string
{
    // Same canonical alphabet/length as normalize_slug(), without rewriting stored slugs.
    return "length($alias.slug) BETWEEN 1 AND 96 AND $alias.slug NOT GLOB '*[^a-z0-9-]*' AND $alias.slug NOT LIKE '-%' AND $alias.slug NOT LIKE '%--%' AND $alias.slug NOT LIKE '%-'";
}
...
function game_provider_approval_save(PDO $pdo, string $key, mixed $approved): void
{
    ...
    if (!$tracked) { require_once __DIR__ . '/../../includes/indexing-queue.php'; indexing_content_changed($pdo, 'game'); $tracked=true; }
...
function game_visibility_refresh(): void
{
    ...
    $output = env_value('GAME_SITEMAP_OUTPUT_DIR') ?: site_root_path();
    require_once __DIR__ . '/../../includes/indexing-queue.php';
    indexing_content_changed(blogs_pdo(), 'game');
    try { generate_game_sitemaps(games_db_path(), $output); }
