...
function fetch_all_games(string $url, ?int $limit = null, int $maxPages = 10000): array
{
    ...
    for ($page = 1; $page <= $maxPages && $currentUrl !== null; $page++) {
        if (!games_enabled()) { echo "Games module disabled; stopping import.\n"; break; }
...
if (realpath($_SERVER['SCRIPT_FILENAME'] ?? '') !== __FILE__ && (!defined('GAMES_SCRIPT_ENTRY') || GAMES_SCRIPT_ENTRY !== basename(__FILE__))) return;

if (!games_enabled()) { echo "Games module disabled; skipping job.\n"; exit(0); }
