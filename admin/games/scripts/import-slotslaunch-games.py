...
LOG_PATH = Path(os.environ.get("APP_STORAGE_DIR") or ROOT / "storage") / "import-slotslaunch-games.log"
...
def main() -> int:
    ...
    # PHP runtime initialization already migrated and initialized games.sqlite.
    ...
    for api_id in queued_ids:
        if conn.execute('SELECT enabled FROM game_module_settings WHERE id=1').fetchone()[0] != 1:
            print('Games module disabled; stopping import.')
            break
        try:
            conn.execute("BEGIN")
