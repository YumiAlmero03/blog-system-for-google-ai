...
function index_checker_config(): array
{
    ...
    return [
        ...
        'property'=>env_value('GSC_PROPERTY') ?: seo_settings()['website_url'],
...
function index_checker_http(string $url, ?string $body = null, array $headers = [], int $maxBytes = 16777216): array
{
...
{
    static $cached=[];
    $path=realpath($config['credentials']); $root=realpath(dirname(__DIR__));
    if (!$path || !is_file($path) || !is_readable($path) || str_starts_with($path,$root.DIRECTORY_SEPARATOR)) throw new RuntimeException('Configure a readable service-account JSON file outside the public site directory.');
    if (isset($cached[$path]) && $cached[$path]['expires']>time()+60) return $cached[$path]['token'];
    $json=file_get_contents($path); $key=is_string($json) ? json_decode($json,true) : null;
    if (!is_array($key) || ($key['type'] ?? '')!=='service_account' || !is_string($key['private_key'] ?? null) || !filter_var($key['client_email'] ?? '',FILTER_VALIDATE_EMAIL)) throw new RuntimeException('Invalid service-account configuration.');
    $b64=static fn(string $v): string => rtrim(strtr(base64_encode($v),'+/','-_'),'=');
    $now=time();
    $jwt=$b64(json_encode(['alg'=>'RS256','typ'=>'JWT'])).'.'.$b64(json_encode(['iss'=>$key['client_email'],'scope'=>'https://www.googleapis.com/auth/webmasters.readonly','aud'=>'https://oauth2.googleapis.com/token','iat'=>$now,'exp'=>$now+3600]));
    if (!@openssl_sign($jwt,$signature,$key['private_key'],OPENSSL_ALGO_SHA256)) throw new RuntimeException('Service-account signing failed.');
    ...
    if ($response['status']!==200 || !is_string($data['access_token'] ?? null)) throw new RuntimeException('Google authentication failed; check API enablement and service-account configuration.');
    $cached[$path]=['token'=>$data['access_token'],'expires'=>$now+min(3600,(int)($data['expires_in'] ?? 3600))];
    return $cached[$path]['token'];
}
