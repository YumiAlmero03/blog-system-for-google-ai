...
require_project_root() {
  ...
  [ -f "includes/blog-storage.php" ] || fail "Run this script from the project root."
...
create_backup() {
  ...
  BACKUP_DIR="$PROJECT_ROOT/storage/backups/$timestamp"
  umask 077
  mkdir -p "$BACKUP_DIR"
  ...
  copy_tree "$PROJECT_ROOT/uploads" "$BACKUP_DIR/uploads" \
    --exclude '.DS_Store'
  ...
  [ -f "$PROJECT_ROOT/.env" ] && cp -p "$PROJECT_ROOT/.env" "$BACKUP_DIR/config/.env"
...
restore_runtime_data() {
  ...
  if [ -d "$BACKUP_DIR/uploads" ]; then
    ...
    copy_tree "$BACKUP_DIR/uploads" "$PROJECT_ROOT/uploads"
  fi
...
tracked_runtime_path() {
  ...
  git ls-files --error-unmatch "storage" >/dev/null 2>&1 && return 0
...
ensure_clean_code_changes() {
  local changed
  changed="$(git status --porcelain --untracked-files=no -- . ':!storage/**' ':!uploads/**' || true)"
  if [ -n "$changed" ]; then
...
run_optional_tasks() {
  if [ -f "scripts/safe-db-update.php" ]; then
    php scripts/safe-db-update.php --apply
  fi

  if [ -x "scripts/build-static-routes.sh" ]; then
...
fix_permissions() {
  ...
  mkdir -p "$PROJECT_ROOT/uploads" "$db_dir"
  chmod u+rwX,g+rwX "$PROJECT_ROOT/uploads" "$db_dir"
  ...
  if [ "$(id -u)" -eq 0 ] && [ -n "$runtime_owner" ]; then
    chown "$runtime_owner" "$PROJECT_ROOT/uploads" "$db_dir"
