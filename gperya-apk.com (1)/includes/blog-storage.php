...
function blogs_page(int $count, int $page, ?string $category = null): array
{
    ...
    if ($category !== null) {
        $totalStmt = $pdo->prepare('SELECT COUNT(*) FROM blog_posts WHERE category = :category');
        $totalStmt->execute([':category' => $category]);
        $total = (int) $totalStmt->fetchColumn();
        $stmt = $pdo->prepare('SELECT * FROM blog_posts WHERE category = :category ORDER BY updated_at DESC, created_at DESC LIMIT :limit OFFSET :offset');
        $stmt->bindValue(':category', $category, PDO::PARAM_STR);
    } else {
        $total = (int) $pdo->query('SELECT COUNT(*) FROM blog_posts')->fetchColumn();
        $stmt = $pdo->prepare('SELECT * FROM blog_posts ORDER BY updated_at DESC, created_at DESC LIMIT :limit OFFSET :offset');
    }
...
function blogs_upsert_with_pdo(PDO $pdo, array $blog): array
{
    $stmt = $pdo->prepare(
        'INSERT INTO blog_posts (
            id, slug, title, category, author, excerpt, content, featured_image, date_label, created_at, updated_at
        ) VALUES (
            :id, :slug, :title, :category, :author, :excerpt, :content, :featured_image, :date_label, :created_at, :updated_at
        )
