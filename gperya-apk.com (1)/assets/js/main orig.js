...
function initCarousel() {
  ...
  carouselContainers.forEach(container => {
    ...
    // Clone set before and set after for infinite looping
    ...
    track.insertBefore(prefixFragment, track.firstElementChild);
    track.appendChild(suffixFragment);
    ...
    const measureWidth = () => {
      ...
      if (allCards.length >= originalCount * 3) {
        const firstSetItem = allCards[0];
        const secondSetItem = allCards[originalCount];
        if (firstSetItem && secondSetItem) {
          singleSetWidth = secondSetItem.offsetLeft - firstSetItem.offsetLeft;
        }
      }
      if (!singleSetWidth || singleSetWidth <= 0) {
        const gap = 16;
        const cardW = originalCards[0] ? originalCards[0].offsetWidth : 220;
    ...
    // Ensure DOM render before setting scroll position
    ...
    const checkInfiniteBounds = () => {
      if (isResetting || !singleSetWidth) return;

      const currentScroll = track.scrollLeft;

      // If scrolled past 2 full sets (into 3rd set suffix)
      ...
      // If scrolled before 1st full set (into 1st set prefix)
    ...
    track.addEventListener('scroll', checkInfiniteBounds, { passive: true });
    ...
    if (nextBtn) {
      nextBtn.addEventListener('click', (e) => {
        e.preventDefault();
        track.style.scrollBehavior = 'smooth';
        if (track.scrollLeft >= singleSetWidth * 2 - 30) {
          track.style.scrollBehavior = 'auto';
          track.scrollLeft -= singleSetWidth;
          track.style.scrollBehavior = 'smooth';
        }
...
function initSpinWheelWidget() {
  ...
  function handleSpin() {
    ...
    // Spin between 5 to 8 full rotations plus random segment offset
    const prizeIndex = Math.floor(Math.random() * 6);
    const extraDegrees = (5 * 360) + (prizeIndex * 60) + 30;
...
function initPragmaticDemoSection() {
  ...
  const demoData = {
    'gates-olympus': {
      ...
      iframeUrl: 'https://demogamesfree.pragmaticplay.net/gs2c/openGame.do?gameSymbol=vs20olympgate&websiteUrl=https%3A%2F%2Fwww.pragmaticplay.com&jurisdiction=99'
    },
    'sweet-bonanza': {
  ...
  sections.forEach(section => {
    ...
    tabs.forEach(tab => {
      tab.addEventListener('click', (e) => {
        ...
        if (iframeEl) iframeEl.src = data.iframeUrl;
      });
    });
  });
}
