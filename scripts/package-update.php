...
try {
    ...
    if ($argc>2) throw new RuntimeException('Usage: sh scripts/package-update.sh [deployed-commit-or-branch]');
    ...
    if (!is_dir('update-packages') && !mkdir('update-packages',0700)) throw new RuntimeException('Cannot create package directory.');
    $prefix='update-packages/site-update-'.gmdate('Ymd-His').'-'.bin2hex(random_bytes(3));
    $zip=new ZipArchive();
