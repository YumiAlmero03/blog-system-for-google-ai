...
try {
    mkdir($temp, 0700);
    foreach (['includes', 'api'] as $directory) {
    ...
    foreach (['missing', 'empty'] as $state) {
        if ($state === 'empty') { fresh_remove($temp . '/storage'); mkdir($temp . '/storage', 0700); }
        fresh_run('require "includes/blog-storage.php"; blogs_pdo();');
        fresh_check(is_file($temp . '/storage/blogs.sqlite'), "$state storage: database created");
        fresh_check(!is_dir($temp . '/storage/api-cache'), 'Cache must be lazy');
        foreach (['login-attempts.min.json', 'customer-tickets.json', 'playnow-clicks.json', 'backups'] as $name) {
            fresh_check(!file_exists($temp . '/storage/' . $name), "$name must be lazy");
        }
        $counts = json_decode(fresh_run('require "includes/blog-storage.php"; $pdo=blogs_pdo(); $counts=[]; foreach (["admin_users","writer_social_links","blog_posts","blog_engagements","games","game_providers","game_types","game_themes","game_theme_links"] as $table) $counts[$table]=$pdo->query("SELECT COUNT(*) FROM $table")->fetchColumn(); echo json_encode($counts);'), true, 512, JSON_THROW_ON_ERROR);
        ...
        fresh_check(is_dir($temp . '/storage/api-cache'), 'List cache created on demand');
        fresh_check(fresh_run('require "includes/admin-support-data.php"; echo json_encode([admin_ticket_rows(),admin_chat_rows("")]);') === '[[],[]]', 'Empty support lists');
        fresh_run('require "includes/blog-storage.php"; blogs_pdo()->exec("UPDATE app_settings SET setting_value=\'preserved\' WHERE setting_key=\'website_title\'"); blogs_pdo()->exec("INSERT INTO games(api_id,name,slug) VALUES(123,\'Preserved\',\'preserved\')");');
        fresh_check(fresh_run('require "includes/blog-storage.php"; $pdo=blogs_pdo(); echo $pdo->query("SELECT setting_value FROM app_settings WHERE setting_key=\'website_title\'")->fetchColumn(), ":", $pdo->query("SELECT name FROM games WHERE api_id=123")->fetchColumn();') === 'preserved:Preserved', 'Existing data survives another process bootstrap');
    }
