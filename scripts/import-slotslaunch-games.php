...
function http_json(string $url): array
{
    ...
    if (function_exists('curl_init') && (str_starts_with($url, 'http://') || str_starts_with($url, 'https://'))) {
        ...
        if (!is_string($raw) || $raw === '') {
            throw new RuntimeException('SlotsLaunch API returned an empty response.' . ($error !== '' ? ' cURL error: ' . $error : ''));
        }

        if ($status >= 400) {
            throw new RuntimeException('SlotsLaunch API returned HTTP ' . $status . ': ' . substr($raw, 0, 300));
        }

        $decoded = json_decode($raw, true);
        if (!is_array($decoded)) {
            throw new RuntimeException('SlotsLaunch API returned invalid JSON: ' . substr($raw, 0, 300));
        }
    ...
    if (!is_string($raw) || $raw === '') {
        throw new RuntimeException('SlotsLaunch API returned an empty response.');
    }
...
$pdo = blogs_pdo();
$result = import_fetched_games($pdo, $url, $limit, $maxPages);
